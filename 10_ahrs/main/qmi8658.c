...
#define STATIC_ACC_VAR_MAX   0.001f   /* g² */
#define STATIC_AMAG_TOL      0.2f     /* g */
#define STATIC_GYR_NORM_MAX  1.5f     /* dps */

/* Bias sanity: reject batch mean deviating > 5 dps from current bias */
#define BIAS_STEP_MAX        0.5f

static i2c_master_dev_handle_t s_dev = NULL;

/* 10-sample history for static detection */
...
/* Batch accumulator for in-motion bias update */
static float s_batch_sum[3] = {0};
static int   s_batch_count  = 0;

/* 5-sample averaging accumulators */
...
void qmi8658_init(i2c_master_dev_handle_t dev)
{
    ...
    write_reg(REG_CTRL2, 0x13);  /* accel ±4g, 1000Hz ODR */
    write_reg(REG_CTRL3, 0x63);  /* gyro ±1024dps, 1000Hz ODR */
...
void qmi8658_calibrate_bias(void)
{
    ...
    printf("INFO: gyro bias calibration started (%d samples)...\n", GYRO_BIAS_SAMPLES);
    ...
    while (count < GYRO_BIAS_SAMPLES && attempts < GYRO_BIAS_SAMPLES * 4) {
        ...
        uint8_t raw[14];
        if (!read_burst(REG_TEMP_L, raw, 14)) continue;
...
static bool detect_static(void)
{
    ...
    /* Accel: only check Z-axis variance — XY polluted by vibration */
    float az_var = 0;
    for (int i = 0; i < HISTORY_LEN; i++) {
    ...
    /* Gyro: Z-axis mean magnitude + full 3-axis norm */
...
bool qmi8658_read(qmi8658_data_t *out)
{
    ...
    uint8_t raw[14];
    if (!read_burst(REG_TEMP_L, raw, 14)) return false;
    ...
    /* Update history for static detection */
    ...
    /* Update bias (uses raw gyro before subtraction) */
...
bool qmi8658_bias_ready(void) { return s_bias_ready; }

void qmi8658_get_bias(float *bx, float *by, float *bz)
{
    *bx = s_bias[0];
    *by = s_bias[1];
    *bz = s_bias[2];
