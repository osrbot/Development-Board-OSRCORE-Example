---
title: 固件烧录
description: 使用浏览器完成 OSRBOT 设备的免 BOOT app 更新或 BOOT 恢复出厂烧录。
---
...
- **免 BOOT 更新**：设备正常开机，不需要进入 BOOT，只更新 app 固件；前提是当前运行的固件支持 `fw` 更新协议。远端固件可以直接从当前仓库例程中选择；自定义 URL 必须填写完整 HTTPS 地址。
